...
if __name__ == "__main__":
    ...
    img_filtered = cv2.erode(img_filtered, None, iterations=1)
    img_filtered = cv2.dilate(img_filtered, None, iterations=1)
    ...
    min_height_ratio = 0.3 # % of the image's height a contour must be to be considered relevant
    ...
    for c in cnts:
        ...
        # if (w >= min_width_ratio*img_width or h >= min_height_ratio*img_height):
        # Check if the height of the contour is likely to be a character
    ...
    # cv2.imshow('image', cv2.cvtColor(img_filtered, cv2.COLOR_GRAY2BGR))
    # img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
